...
iterates_solvers = Dict{String, Vector{Float64}}()
stats_slovers = Dict{String, Any}()
...
#    end
#end
#x_a_full = Vector{Vector{Float64}}()
#global xanalysis .= copy(xa)
#push!(x_a_full, xanalysis)


# Observations
#y_obs = reshape(y, length(obs.space_inds), length(obs.time_inds))
...
#or name in ["minres_qlp", "minres", "cg_lanczos", "cg"]
#    plot!(times, [x[idx] for x in trajectories[name]], label=name, lw=2)
#end
#scatter!(obs.time_inds, y_obs[1, :], label="Observations", ms=4, c=:black)
#xlabel!("Time step")
#ylabel!("State x[$idx]")
