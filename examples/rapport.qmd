...
1. Mise à jour de la hauteur de la surface de l'eau $\zeta$  à une position donnée indexée par $j$ :
$$
\zeta_j^{(new)} = \zeta_j - \frac{\text{dt} \cdot h}{x_u[j+1] - x_u[j]} (u[j+1] - u[j])
$$
...
$$
u_j^{(new)} = u_j - \frac{\text{dt} \cdot g}{x_r[j] - x_r[j-1]} (\zeta[j] - \zeta[j-1])
$$
...
$$
y_i = \mathcal{H}(\mathcal{M}_{t_0, t_i}(xt))  + \epsilon_{\text{obs}}
$$
